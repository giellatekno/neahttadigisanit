...
def format_source_sma(ui_lang, e, target_lang):
    ...
    if _lemma_ref:
        _link_targ = "/detail/%s/%s/%s.html" % ("sma", target_lang, _lemma_ref)
        _lemma_ref_link = '<a href="%s"/>%s</a>' % (_link_targ, _lemma_ref)
        _lemma_ref_link = '<span class="see_also"> → ' + _lemma_ref_link
        _lemma_ref_link += "</span>"
