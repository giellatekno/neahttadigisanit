...
from neahtta.utils.data import flatten
...
class EntryNodeIterator:
    ...
    def l_node(self, entry):
        ...
        return lemma, pos, context, typ, hid

    # anders: rewrote logic
    ...
    def tg_nodes(self, entry):
        ...
        # pair format may be disrupted by disallowing fallback?
        target_lang = self.query_kwargs.get("target_lang", False)
        if not target_lang or len(entry.xpath("mg/tg/@xml:lang")) == 0:
            ts = entry.findall("mg/tg/t")
            tgs = entry.findall("mg/tg")
        else:
            ts = entry.xpath(f"mg/tg[@xml:lang='{target_lang}']/t")
            tgs = entry.xpath(f"mg/tg[@xml:lang='{target_lang}']")
        return tgs, ts
...
class FrontPageFormat(EntryNodeIterator):
    def clean_tg_node(self, e, tg):
        ...
        target_formatter = partial(
            lexicon_overrides.format_target,
            self.query_kwargs.get("source_lang"),
            self.query_kwargs.get("target_lang"),
            ui_lang,
            e,
        ...
        def add_link(_p):
            ...
            if (_from_l, _to_l) not in current_app.config.dictionaries and (
                _from_l,
                _to_l,
            ) in current_app.config.variant_dictionaries:
            ...
            _url = [
                ...
                self.query_kwargs.get("target_lang"),
                src_lang,
                f"{_p}.html?no_compounds=true&lemma_match=true",
            ]
            _url = "/" + "/".join(_url)
            link = f"<a href='{_url}'>{_p}</a>"
            return link
    ...
    def clean(self, e):
        ...
        if lemma and lemma_pos:
            default_format = "%s (%s)" % (
                lemma,
                tagfilter(lemma_pos, source_lang, target_lang),
            )
