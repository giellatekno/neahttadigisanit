...
class ParadigmException(Exception):
    ...
    def __repr__(self):
        return f"{self.message} (in ...{self.template})"
...
class Value:
    ...
    def compare_value(self, tag_list, lemma):
        ...
        if "*" in search_v:
            ...
            return current_form_tag.startswith(
                start_search_pred
            ) and current_form_tag.endswith(end_search_pred)
...
class TableParser:
    ...
    def header_positions(self):
        ...
        heads = (i for i, c in enumerate(self.header) if c == self.COLUMN_DELIM)
    ...
    def lines(self):
        ...
        lines = (a.strip() for a in self.raw.splitlines())
        return [L for L in lines if L]

    # TODO: paradigm
    def __init__(self, _str, options=None):
        self.raw = _str
    ...
    def validate(self):
        ...
        if not self.lines:
            ...
            lengths = set(len(L) for L in self.lines)
