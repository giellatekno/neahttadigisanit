...
class ParadigmLanguagePairSearchView(DictionaryView, SearcherMixin):
    ...
    def get_paradigms(self, _from, lemma):
        ...
        Returns:
            flask.Response
        """
        from utils.debug import TRACE, DEBUG
        ...
        cache_key = "+".join([key for key in [_from, lemma, pos_filter, e_node] if key])
    ...
    def search_to_paradigm(self, lookup_value):
        ...
        DEBUG("Calling into do_search_to_obj with generate=True now")
        ...
        return [
            paradigm
            for _, _, paradigm, _ in search_result_obj.entries_and_tags_and_paradigms
        ]
        ...
        return json_response(
            {
                ...
                "input": {"lemma": lemma, "pos": request.args.get("pos", False)},
            },
