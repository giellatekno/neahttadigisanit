...
@blueprint.context_processor
def text_tv():
    return {"text_tv": "text_tv" in session}
...
def add_languages():
    ...
    if current_app.config.hidden_locales:
        locales = [
            L
            for L in current_app.config.locales_available
...
def language_pairs_grouped_by_source():
    return dict(
        language_pairs_grouped_by_source=current_app.config.pair_definitions_grouped_source_locale()
    )
...
def footer_template():
    ...
    footer_template = current_app.lexicon_templates.render_individual_template(
        _from,
        "footer.template",
        **{"current_locale": get_locale(), "_from": _from, "_to": _to},
    )
