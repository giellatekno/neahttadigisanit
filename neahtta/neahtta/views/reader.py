...
from neahtta.bookmarklet_code import generate_bookmarklet_code
from neahtta.i18n.utils import iso_filter
from neahtta.utils.json import fmtForCallback
from neahtta.utils.logger import logSimpleLookups
...
def json_response(data, pretty=False):
    ...
    Args:
        ...
        pretty (bool): if True, make json human friendly
...
def lookupWord(from_language, to_language):
    ...
    if lookup_key is False or not lookup_key.strip():
        return json_response(
            {"result": [], "tags": [], "tag_msg": "", "success": False}
        )
    ...
    for lookup in lookups:
        ...
        def filterPOSAndTag(analysis):
            return analysis.lemma, " ".join(analysis.tag)
...
def ie8_instrux_json():
    ...
    return Response(response=formatted, status=200, mimetype="application/json")
...
def fetch_messages(locale):
    ...
    # anders: what exception are we catching here?
    except Exception:
        _pofile = False

    if _pofile:
        jsentries = filter(
            lambda x: any(".js" in a[0] for a in x.occurrences), list(_pofile)
        )
    ...
    if override_symbol:
        jsentries_dict["Á"] = override_symbol
...
def bookmarklet_configs():
    ...
    new_group = False
    for grouper, group in pair_groups.items():
        for (_from, _to), pair_options in group:
            ...
            if grouper != new_group:
                group = {
                    "iso": grouper,
                    "locale_name": str(NAMES.get(grouper)),
                    "self_name": str(LOCALISATION_NAMES_BY_LANGUAGE.get(grouper)),
                }
            else:
                group = False
            ...
            dictionaries.append(
                {
                    "from": {"iso": _from, "name": str(NAMES.get(_from))},
                    "to": {"iso": _to, "name": str(NAMES.get(_to))},
            ...
            _has_variant = current_app.config.pair_definitions.get(
                (_from, _to), {}
                        ...
                        dictionaries.append(
                            {
                                "from": {
                                    ...
                                    "name": "%s (%s)"
                                    % (
                                        str(NAMES.get(_from, _from)),
                                        _(variant.get("description", "")),
                                    ),
                                },
                                "to": {
                                    "iso": _to,
                                    "name": str(NAMES.get(_to)),
                                },
