...
definition_exists_tests = [
    ...
    (("kpvS", "fin"), "морсјаслыԍ", "mehu"),
    (("kpvS", "fin"), "морсъяслысь", "mehu"),
    (("kpvS", "fin"), "мортлаԋ", "ihminen"),
    (("kpvS", "fin"), "бюҗет", "budjetti"),
]

paradigm_generation_tests = [
    # source, target, lemma, error_msg, paradigm_test
    ###  - V:
    ("kpv", "fin", "мунны", "kpv verbs not generating", form_contains(set(["муна"]))),
    ###  - N + context="bivttas":  heittot
    ###     - http://localhost:5000/detail/sme/nob/heittot.html
    ("kpv", "fin", "морс", "kpv nouns not generating", form_contains(set(["морслӧн"]))),
]
