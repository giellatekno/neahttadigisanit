...
DEFAULT_SME_TAGSETS = {
    ...
    "pron_type": ["Dem", "Indef", "Interr", "Pers", "Recipr", "Refl", "Rel"],
    "type": [
        "NomAg",
        "G3",
        "G7",
...
def make_lemmatize_answer(wordform, tags):
    ...
    return [
        Lemma(
            ...
            _input=wordform,
            tool=fst_tool(),
            tagsets=Tagsets(DEFAULT_SME_TAGSETS),
        )
        for tag in tags
    ]
...
class TestHFST(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            (
                "known analysis",
                "guolit\tguolli+N+Sg+Nom\t0,000000",
                ("guolit", "guolli+N+Sg+Nom"),
            ),
            (
                "unknown analysis",
                "geahppánit+V+Ind+Prs+Sg1\tgeahppánit+V+Ind+Prs+Sg1+?\tinf",
                ("geahppánit+V+Ind+Prs+Sg1", "geahppánit+V+Ind+Prs+Sg1+?\t+?"),
            ),
        ]
    )
    def test_tagprocessor(self, name, analysis_line, result):
        """Test the tagprocessor."""
        self.assertEqual(fst_tool().tag_processor(analysis_line), result, msg=name)
...
guollái\tguollái+A+Sg+Nom\t0,000000"""
    ...
    def test_lookup(self):
        ...
        with app.app_context():
            got = fst_tool().lookup(["guollái", "viežži"])
            wanted = [
                ("guollái", ["guolli+N+Sg+Ill", "guollái+A+Sg+Nom"]),
                (
                    "viežži",
                    [
                        "viežžat+V+TV+PrsPrc",
                        "viežžat+V+TV+Imprt+Du2",
                        "viežžat+V+TV+Der/NomAg+N+Sg+Gen",
                        "viežžat+V+TV+Der/NomAg+N+Sg+Acc",
                        "viežžat+V+TV+Der/NomAg+N+Sg+Nom",
                    ],
                ),
            ]
    ...
    def test_inverse_lookup(self, name, lemma, tags, wanted):
        ...
        with app.app_context():
            self.assertEqual(fst_tool().inverselookup(lemma, tags), wanted, msg=name)
...
guolli+N+Pl+Com"""
    ...
    def test_lemmatize(self, name, wordform, kwargs, tags):
        ...
        with app.app_context():
            got = app.config.morphologies["sme"].morph_lemmatize(wordform, **kwargs)
            wanted = make_lemmatize_answer(wordform, tags)
            self.assertEqual(got, wanted, msg=name)

    @parameterized.expand(
        [
            ("no unknown", [("a", ["b", "c"])], False),
        ...
        [
            ...
            (
                ...
                [
                    ...
                    "guollebiebman+N+Sg+Gen",
                    "guollebiebman+N+Sg+Acc",
                ],
                [
                    "guollebiebman+N+Sg+Gen",
                    "guollebiebman+N+Sg+Acc",
    ...
    def test_check_if_lexicalized(self, name, wordform, input_list, wanted):
        ...
        got = app.config.morphologies["sme"].check_if_lexicalized(wordform, input_list)

        self.assertListEqual(got, wanted, msg=name)

    @parameterized.expand(
        [("no dupes", ["a", "b"], ["a", "b"]), ("has dupes", ["a", "a"], ["a"])]
    )
    ...
    @parameterized.expand(
        [
            ...
            ("first element is list", [["d", "e"], "f"], ["a", "b"], ["d", "e", "f"]),
        ]
    )
        ...
        [
            ...
            (
                ...
                [
                    ...
                    "guollebiebman+N+Sg+Nom",
                    "guolli+N+Cmp/SgNom",
                    "biebmat+V+TV+Der/NomAct+N+Sg+Gen",
                    "guolli+N+Cmp/SgNom",
                    "biebmat+V+TV+Der/NomAct+N+Sg+Nom",
                    "guolli+N+Cmp/SgNom",
                    "biebman+N+Sg+Gen+Allegro",
                    "guolli+N+Cmp/SgNom",
                    "biebman+N+Sg+Nom",
    ...
    def test_split_on_compound(self, name, input_list, split_compounds, wanted):
        """Test the morphology split on compound function."""
        self.assertListEqual(
            app.config.morphologies["sme"].split_on_compounds(
                input_list, split_compounds
            ),
            wanted,
            msg=name,
        )
    ...
    def test_rearrange_on_count(self, name, analyses, wanted):
        ...
        self.assertListEqual(
            app.config.morphologies["sme"].rearrange_on_count(analyses),
            wanted,
            msg=name,
        )
    ...
    @parameterized.expand(
        [
            ...
            (
                "Gerd",
                [
                    "Gerd+N+Prop+Sem/Fem+Attr",
                    "Gerd+N+Prop+Sem/Fem+Sg+Gen",
                    "Gerd+N+Prop+Sem/Fem+Sg+Acc",
                    "Gerd+N+Prop+Sem/Fem+Sg+Nom",
                ],
                [
                    "Gerd+N+Prop+Sem/Fem+Attr",
                    "Gerd+N+Prop+Sem/Fem+Sg+Gen",
            ...
            (
                "buoremus",
                ["buorre+A+Superl+Attr", "buorre+A+Superl+Sg+Nom"],
                ["buorre+A", "Superl+Attr", "Superl+Sg+Nom"],
            ),
            ("vuojedettiin", ["vuodjit+V+TV+Ger"], ["vuodjit+V+TV", "Ger"]),
            (
                "biepmadettiin",
                ["biebmat+V+TV+Ger", "biebmat+V+TV+Der/d+V+Ger", "biepmadit+V+TV+Ger"],
                ["biebmat+V+TV", "Ger", "Der/d+V", "biepmadit+V+TV"],
            ),
            (
                "wordform",
...
class TestMorphoLexicon(unittest.TestCase):
            ...
            (
                ...
                [
                    ...
                    (
                        "guolli",
                        "fisk",
                        make_lemmatize_answer("guolli", ["guolli+N+Cmp/SgNom"]),
                    ),
                    (
                        "biebman",
                        "ernæring",
                        make_lemmatize_answer(
                            "biebman", ["biebman+N+Sg+Nom", "biebman+N+Sg+Gen+Allegro"]
                        ),
                    ),
                    (
                        "biebmat",
                        "fø",
                        make_lemmatize_answer("biebmat", ["biebmat+V+TV"]),
                    ),
                    (
                        "Der/NomAct",
                        "verbhandlingen",
                        make_lemmatize_answer(
                            "Der/NomAct", ["Der/NomAct+N+Sg+Nom", "Der/NomAct+N+Sg+Gen"]
                        ),
                    ),
                ],
            ),
            (
                ...
                [
                    ...
                    (
                        "guoddi",
                        "bærer",
                        make_lemmatize_answer("guoddi", ["guoddi+N+NomAg+Pl+Nom"]),
                    ),
                    (
                        "Der/NomAg",
                        "den som utfører verbet",
                        make_lemmatize_answer("Der/NomAg", ["Der/NomAg+N+Pl+Nom"]),
                    ),
                ],
            ),
            (
                ...
                [
                    ...
                    (
                        "mun",
                        "jeg",
                        make_lemmatize_answer("mun", ["mun+Pron+Pers+Pl1+Nom"]),
                    ),
                ],
            ),
        ]
    ...
    def test_lookup(self, name, wordform, wanted):
        ...
        with app.app_context():
            ...
            results = [make_morpholex_result(result) for result in result_list]
